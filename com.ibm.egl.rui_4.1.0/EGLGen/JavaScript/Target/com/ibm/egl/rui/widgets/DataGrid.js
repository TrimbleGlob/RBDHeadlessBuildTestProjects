...
egl.defineRUIWidget('com.ibm.egl.rui.widgets', 'DataGrid', {
	...
	"showLoadingDiv": function() {
		try {
			...
			if ( (uiCount >= 1) ) {
				...
				egl.checkNull(this.loadingDiv).setPaddingTop(Math.max( egl.convertDecimalToInt(new egl.javascript.BigDecimal(egl.divide(egl.checkNull(this.ui).getPixelHeight(), 2)).subtract(egl.convertIntegerToDecimal(12, egl.javascript.BigDecimal.prototype.NINES[3]))) ,0 ));
				...
				egl.checkNull(this.loadingDiv).setPaddingLeft(Math.max( egl.convertDecimalToInt(new egl.javascript.BigDecimal(egl.divide(egl.checkNull(this.ui).getChildren()[egl.checkNull(this.ui).getChildren().checkIndex((1)-1)].getPixelWidth(), 2)).subtract(egl.convertIntegerToDecimal(20, egl.javascript.BigDecimal.prototype.NINES[3]))) ,0 ));
				egl.checkNull(this.loadingDiv).setHeight((new egl.javascript.BigDecimal(egl.divide(egl.checkNull(this.ui).getPixelHeight(), 2)).add(egl.convertIntegerToDecimal(12, egl.javascript.BigDecimal.prototype.NINES[3]))).toString());
			}
			else {
	...
	"dragColumn": function( w, dropw, dragX, dragY ) {
		var column = 0;
		column = egl.convertAnyToInt(w.getAttribute("column" ),false);
		var target = 0;
	...
	"gotoPage": function( pageIndex ) {
		...
		var oldPageIndex = 0;
		oldPageIndex = egl.convertDecimalToInt(new egl.javascript.BigDecimal(egl.divide((this._startRow - 1), this.pageSize)).add(egl.convertIntegerToDecimal(1, egl.javascript.BigDecimal.prototype.NINES[3])),egl.createRuntimeException);
	...
	"getPageCount": function() {
		var $result = 0;
		$result = egl.convertDecimalToInt(new egl.javascript.BigDecimal(egl.divide((this.data.getSize() - 1), this.pageSize)).add(egl.convertIntegerToDecimal(1, egl.javascript.BigDecimal.prototype.NINES[3])),egl.createRuntimeException);
		return $result;
	}
	,
	"getCurrentPageIndex": function() {
		var $result = 0;
		$result = egl.convertDecimalToInt(new egl.javascript.BigDecimal(egl.divide((this._startRow - 1), this.pageSize)).add(egl.convertIntegerToDecimal(1, egl.javascript.BigDecimal.prototype.NINES[3])),egl.createRuntimeException);
		return $result;
	}
	,
	...
	"gotoEnd": function( e ) {
		...
		oldPageIndex = egl.convertDecimalToInt(new egl.javascript.BigDecimal(egl.divide((this._startRow - 1), this.pageSize)).add(egl.convertIntegerToDecimal(1, egl.javascript.BigDecimal.prototype.NINES[3])),egl.createRuntimeException);
		var pageCount = 0;
		pageCount = egl.convertDecimalToInt(new egl.javascript.BigDecimal(egl.divide((this.data.getSize() - 1), this.pageSize)).add(egl.convertIntegerToDecimal(1, egl.javascript.BigDecimal.prototype.NINES[3])),egl.createRuntimeException);
		var leftOver = 0;
	...
	"updateHighlights": function() {
		...
		divCount = divs.getSize();
		{
			...
			{
				var row = 0;
				row = egl.convertAnyToInt(egl.checkNull(divs)[divs.checkIndex((n)-1)].getAttribute("row" ),false);
				if ( (row > 0) ) {
					var selected = false;
		...
		if ( this.showCheckBoxes ) {
			...
			checkBoxCount = checkBoxes.getSize();
			{
				...
				{
					var row = 0;
					row = egl.convertAnyToInt(egl.checkNull(checkBoxes)[checkBoxes.checkIndex((c)-1)].getAttribute("row" ),false);
		...
		inputCount = inputs.getSize();
		{
			...
			for ( ; (n <= inputCount); n = (n + 1) )
			{
				if ( (egl.checkNull(inputs)[inputs.checkIndex((n)-1)].getID() == checkBox.getID()) ) {
					var isChecked = false;
					isChecked = (("" + egl.convertAnyToString(checkBox.getAttribute("checked" ),false)) == "true");
					var row = 0;
					row = egl.convertAnyToInt(checkBox.getParent().getParent().getAttribute("row" ),false);
	...
	"selectRow": function( div, selected ) {
		var row = 0;
		row = egl.convertAnyToInt(div.getAttribute("row" ),false);
		if ( (row > 0) ) {
			if ( selected ) {
	...
	"clickGrid": function( e ) {
		...
		while ( ((widget.getClass() != "EglRuiDataGrid") && (egl.convertAnyToInt(widget.getAttribute("column" ),false) == 0)) ) {
			widget = widget.getParent();
		}
		if ( (widget.getClass() != "EglRuiDataGrid") ) {
			var row = 0;
			row = egl.convertAnyToInt(widget.getAttribute("row" ),false);
	...
	"sortColumn": function( widget ) {
		var column = 0;
		column = egl.convertAnyToInt(widget.getAttribute("column" ),false);
	...
	"getColumn": function( td ) {
		var $result = null;
		var column = 0;
		column = egl.convertAnyToInt(td.getAttribute("column" ),false);
