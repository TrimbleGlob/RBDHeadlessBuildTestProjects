...
egl.defineClass('com.ibm.egl.rui.widgets', 'GridSelector', {
	...
	"setSelected": function( e ) {
		...
		var dataRecord = null;
		dataRecord = egl.boxAny(egl.checkNull(grid).getData()[egl.checkNull(grid).getData().checkIndex((egl.convertAnyToInt(td.getAttribute("rowNumber" ),false))-1)]);
		...
		tds = tr.getElementsByTagName("td" );
		{
			...
			for ( ; (n <= tds.getSize()); n = (n + 1) )
			{
				if ( (egl.checkNull(tds)[tds.checkIndex((n)-1)].getClass() != "EglRuiGridHeader") ) {
					egl.checkNull(tds)[tds.checkIndex((n)-1)].setBackgroundColor( egl.convertAnyToString(egl.checkNull(tds)[tds.checkIndex((n)-1)].getAttribute("originalBG" ),false) );
				}
			}
		}
