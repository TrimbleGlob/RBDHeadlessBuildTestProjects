...
egl.defineRUIWidget('com.ibm.egl.rui.widgets', 'RadioGroup', {
	...
	"setSelected": function( select ) {
		this.selected = select;
		{
			...
			{
				if ( (egl.convertAnyToString(egl.checkNull(this.optionsList)[this.optionsList.checkIndex((opt)-1)].getAttribute("value" ),false) == select) ) {
	...
	"setChangeEvent": function( e ) {
		if ( (this.selectedOption != e.widget) ) {
			...
			this.selected = egl.convertAnyToString(e.widget.getAttribute("value" ),false);
			e.widget =  this;
			{
				var index = 0;
				index = 1;
