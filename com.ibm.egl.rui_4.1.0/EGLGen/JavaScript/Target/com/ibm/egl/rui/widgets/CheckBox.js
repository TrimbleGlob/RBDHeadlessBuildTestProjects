...
egl.defineRUIWidget('com.ibm.egl.rui.widgets', 'CheckBox', {
	...
	"getSelected": function() {
		...
		browser = egl.egl.ui.rui.RUILib['$inst'].getUserAgent();
		if ( (egl.egl.core.$StrLib.indexOf( browser, ("MSIE 7.0")) != 0) ) {
			$result = egl.convertAnyToBoolean(this.check.getAttribute("checked" ),false);
			return $result;
		}
		else {
			$result = (egl.convertAnyToString(this.check.getAttribute("checked" ),false) == "true");
			return $result;
		}
		return $result;
	}
	...
	"getWidgetOrientation": function() {
		var $result = "";
		if ( (egl.convertAnyToString(this.span.getAttribute("dir" ),false) == "rtl") ) {
			$result = "RTL";
			return $result;
		}
		else {
			if ( (egl.convertAnyToString(this.span.getAttribute("dir" ),false) == "ltr") ) {
				$result = "LTR";
				return $result;
			}
			else {
